# Side Quest Week 3: Full Fighting Game

## Setup and Interaction Instructions

A two-player fighting game where each player can move, attack, and block using separate keyboard controls.
Defeat your opponent through attacking, positioning, and defense, then press ENTER to start a new match or rematch.

**Player 1 Controls:**

- Move: A / D
- Attack: F
- Block: G

**Player 2 Controls:**

- Move: Arrow Keys
- Attack: K
- Block: L

Press **ENTER** to start or rematch.

## Assets

| File                                      | Source                                                                              |
| ----------------------------------------- | ----------------------------------------------------------------------------------- |
| `assets/sounds/win_sound.mp3` [1]         | floraphonic, You Win Sequence — pixabay.com                                         |
| `assets/sounds/missSwing_sound.mp3` [2]   | floraphonic, Near Miss Swing Woosh — pixabay.com                                    |
| `assets/fonts/fastlynk.ttf` [3]           | Gassstype Studio, Fastlynk Font — Dafont.com                                        |
| `assets/sounds/portal_bg.jpeg` [4]        | Chris Kohler, Review: Portal 2 Marries Indie Design, Blockbuster Budget — wired.com |
| `assets/sounds/punch_sound.mp3` [5]       | soraatwod, Punch — pixabay.com                                                      |
| `assets/sounds/auraPower_bgMusic.mp3` [6] | Walen, Aura Power — freetouse.com                                                   |

## References

[1]floraphonic. 2024. You Win Sequence 1 | Royalty-free Music. Pixabay.com. Retrieved from https://pixabay.com/sound-effects/musical-you-win-sequence-1-183948/

[2]floraphonic. 2024. Near Miss Swing Whoosh 18 | Royalty-free Music. Pixabay.com. Retrieved from https://pixabay.com/sound-effects/film-special-effects-near-miss-swing-whoosh-18-233442/

[3]Gassstype Studio. 2023. Fastlynk Font. Dafont.com. Retrieved from https://www.dafont.com/fastlynk.font

[4]Chris Kohler. 2011. Review: Portal 2 Marries Indie Design, Blockbuster Budget. Wired. Retrieved from https://www.wired.com/2011/04/portal-2-review/

[5]soraatwod. 2026. Punch. Pixabay.com. Retrieved from https://pixabay.com/sound-effects/punch-416719/

[6]Walen. 2026. Aura Power. Free To Use. Retrieved from https://freetouse.com/music/walen/aura-power
