...
// ============================================================
function preload() {
  // Load sounds for punch, win, and background music
  punchSound = loadSound("assets/sounds/punch_sound.mp3");
  winSound = loadSound("assets/sounds/win_sound.mp3");
  bgMusic = loadSound("assets/sounds/auraPower_bgMusic.mp3");
...
function drawWinScreen() {
  ...
  // Draw "portal" at the top of the screen for a glow on the win screen
  // Outer ellipse (bright blue)
  fill(0, 150, 255); // bright blue
  noStroke();
  ellipse(width / 2, height / 2 - 150, 300, 120);

  // Inner ellipse (white, smaller)
  fill(255); // white
  ellipse(width / 2, height / 2 - 150, 200, 80);

  // Draws a translucent glow with a trapezoid over the "win" as if a portal is opening for the winner
  noStroke();
  fill(255, 255, 255, 40); // white with low opacity for glow

  beginShape();
  vertex(width / 2 - 100, 70); // top-left
  vertex(width / 2 + 100, 70); // top-right
  vertex(width / 2 + 200, 400); // bottom-right
  vertex(width / 2 - 200, 400); // bottom-left
  endShape(CLOSE);
...
function checkHits() {
  ...
  if (fighter1.isAttacking && !fighter1.hitLanded) {
    ...
    if (dist < fighter2.r + 10) {
      ...
      punchSound.play(); // Play punch sound on hit for extra impact
    } else {
      missSound.play(); // Play miss sound if punch doesn't connect
    }
  }
